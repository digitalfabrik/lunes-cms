...
const initFeedbackDialog = (): void => {
    ...
    const reasonSelect = dialog.querySelector<HTMLSelectElement>("select[name='reason']")!
    const reasonStatuses = reasonSelect.dataset.statuses!.split(" ")

    const showReasonsFor = (status: string): void => {
        reasonSelect.value = ""
        reasonSelect.querySelectorAll("optgroup").forEach(function (group, index) {
            const applies = reasonStatuses[index] === status
            group.hidden = !applies
            // a disabled group cannot be picked, even where `hidden` is ignored
