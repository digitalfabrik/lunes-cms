type DocumentData = {
    word: string
    document_image: Array<{ image: string }>
    audio: string
}
...
*/
function get_document(document_id: string | number): void {
    $.ajax({
        type: "GET",
        url: "/api/document_by_id" + "/" + document_id,
        dataType: "json",
        async: false,
        success: function (data: DocumentData[]) {
...
*/
function display_thumbnail(doc: DocumentData): void {
...
*/
// Called from Django widget onclick attribute (cms/widgets.py)
window.document_overlay = function (event: MouseEvent & { target: HTMLSelectElement }): void {
    if (event.altKey) {
