...
.. list-table::
   ...
   * - ``delivery_beta``
     - Bumps the version, runs tests, publishes a beta package to TestPyPI,
       creates a GitHub pre-release, deploys the user manual.
   * - ``promotion``
...
3. **generate-sbom** — Generates a Software Bill of Materials and commits it to the
   repository. Will be replaced by ``app-toolbelt`` once SBOM support is available.

4. **e2e** — Runs the end-to-end test suite and generates the user manual docs.

5. **build-package** — Builds the Python source distribution and wheel.

6. **publish-beta-package** — Uploads the package to
   `TestPyPI <https://test.pypi.org/project/lunes-cms/>`__ (context: ``pypi-test``).

7. **deploy-user-manual** — Deploys the generated docs to GitHub Pages
   (branch ``gh-pages-user-docs``).

8. **create-release** — Creates a GitHub pre-release for the new tag with auto-generated
   release notes from GitHub (based on ``.github/release.yml``), uploads the built artifact
   as a release asset, and notifies Mattermost (context: ``deliverino``).
